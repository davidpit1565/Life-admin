...
if rg -n "GEMINI_API_KEY" . --glob '!/.git/**' --glob '!.build/**' | rg -v 'docs/GEMINI_CONFIGURATION.md|docs/BUILD_AND_TESTING.md|server/gemini-proxy.js|tests/gemini-integration.js|scripts/qa.sh|scripts/security_scan.sh|.env.example'; then fail=1; fi
