...
public struct ReminderEngine { public init() {} ; public func notificationDates(for item: LifeAdminItem, calendar: Calendar = .current) -> [Date] { guard let due=item.dueDate else { return [] }; return item.reminderOffsets.compactMap { calendar.date(byAdding: .day, value: -$0, to: due) }.filter { $0 > Date(timeIntervalSince1970: 0) }.sorted() } }
